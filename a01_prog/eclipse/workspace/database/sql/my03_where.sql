...
/*
NOT 연산자
해당 연산자는 참이면 거짓, 거짓이면 참으로 처리되어 나타날 필요가 있을 때,
활용한다.
...
/*
날짜에 대한 처리..
입사일이 1987/01/01 에서 1987/12/31 사이에 있는 데이터를 list하세요..
*/
select * from emp
...
*/
-- 관리자 번호가 7902, 7566, 7839인 것을 LIST하세요..
select * from emp
where mgr in (7902, 7566, 7839);
-- 확인예제) 사원이름이 'SMITH', 'WARD', 'JONES' 인 사람 중,
-- 			연봉이 1000미만인 데이터를
--			이름과 연봉을 출력하세요.
...
특정 데이터에 대한 문자열 전체 내용을 정확하게 알지 못할 때, 검색가능한 것은 like연산자와
와일드카드를 활용하는 방법이다..
형식) 검색컬럼명 like '%검색할데이터%';
ex) 사원명이 K가 포함된 데이터를 list하세요
*/
select * from emp
where ename like '%K%';
-- 만약 K로 시작하는 데이터를 list할려면?
select * from emp
where ename like 'K%';
-- 직책중, MAN으로 끝나는 데이터를 list하세요..
select * from emp
where ename like '%MAN';
-- 확인예제) 직책중, S로 시작하는 데이터에서 보너스가 300이거나 400이거나,
--			700인 데이터를 직책, 사원명, 보너스를 list하세요..
...
그럼, 이에 대한 기준되는 컬럼을 정해야 하며, 오름차순인지 내림차순인지를 표현해야 한다.
형식) order by 기준컬럼1 asc/desc
asc(ascending) : 오름차순을 말하며 일반적으로 설정하지 않으면 오름차순으로 출력된다. default
desc(descending) : 내림차순을 말한다. 
*/  
select * from emp
order by sal asc;
select * from emp
order by sal desc;
-- ex) 직책이 MANAGER인 데이터를 입사일 기준으로 오름차순으로 LIST
select * from emp
where job = 'MANAGER'
...
3) 부서번호가 빠른 사원부터 출력하되 같은 부서내 사원은 최근 입사한 사원부터 출력
	부서번호, 입사일, 사원명, 급여순으로 출력
*/
-- 1)
