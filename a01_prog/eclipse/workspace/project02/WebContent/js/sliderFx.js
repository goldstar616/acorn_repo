...
;( function( window ) {
	...
	SliderFx.prototype._init = function() {
		...
		this.itemsImgList = ["../img/daom1.jpg", "../img/daom2.jpg"];
		
		// current and old item夷늮 index
		...
		if( this.itemsCount > 1 ) {
			...
			this.items.forEach( function( item ) {
				...
				var svg = createSvg('<svg xmlns="http://www.w3.org/2000/svg" width="100%" height="95%" viewBox="0 0 80 60" preserveAspectRatio="none" fill="url(#image' + index + ')"><path d="' + self.options.paths.rect + '"/><defs><pattern id="image' + index + '" width="100%" height="95%" viewBox="0 0 80 60" preserveAspectRatio="none"><image xlink:href="' + self.itemsImgList[index] + '" width="100%" height="100%" preserveAspectRatio="none"></image></pattern></defs></svg>');
